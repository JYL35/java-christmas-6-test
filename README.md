...
- 식당 방문 날을 입력 받고 파싱 후 유효성 검사를 받는다.
  - 방문 날은 비어있을 수 없다.
  - 방문 날은 숫자이다.
  - 방문 날은 1~31이다.
- 주문할 메뉴와 개수를 입력 받고 파싱 후 유효성 검사를 받는다.
  - 모든 주문할 메뉴와 개수를 `,`로 분리한다.
  - 주문할 메뉴와 개수는 각각 `-`로 분리한다.
    - 메뉴의 수와 메뉴 개수의 수는 동일해야 한다.
    - 주문할 메뉴는 정해진 메뉴판에 있어야 한다.
    - 주문할 메뉴는 중복되지 않는다.
    - 모든 주문할 메뉴가 음료일 수 없다.
    - 메뉴 개수는 1이상이다.
    - 총 메뉴 개수는 20이하이다.
...
- [x] 미션은 java-christmas-6 저장소를 비공개 저장소로 생성해 시작한다.
